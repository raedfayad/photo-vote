...
function RetryImage({ src, alt, className }) {
  const [attempt, setAttempt] = useState(0)
  const [errored, setErrored] = useState(false)
  return errored ? (
    <div className={`${className} flex flex-col items-center justify-center gap-1 bg-gray-100`}>
      <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
      </svg>
      <button
        onClick={() => { setAttempt(a => a + 1); setErrored(false) }}
                      ...
                      <div key={v.id} className="relative">
                        ...
                        >
                          <div className="aspect-[4/3] bg-gray-100">
                            <RetryImage src={v.url} alt={`Version ${label}`} className="w-full h-full object-cover" />
                          </div>
